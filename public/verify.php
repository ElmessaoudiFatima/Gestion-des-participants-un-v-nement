...
if (isset($_GET['token']) && !empty($_GET['token'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT id, email, nom, prenom, is_verified, created_at FROM users WHERE verification_token = ?");
        $stmt->execute([$token]);
        $user = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($user) {
            if ($user['is_verified']) {
                $message = "Ce compte a deja ete verifie. Vous pouvez vous connecter.";
                $success = true;
            } else {
                ...
                if ($hours > 24) {
                    $message = "Ce lien de verification a expire. Veuillez vous reinscrire.";
                } else {
                  
                    $stmt = $pdo->prepare("UPDATE users SET is_verified = 1, verification_token = NULL WHERE id = ?");
                    $stmt->execute([$user['id']]);

                    $message = "Felicitations ! Votre compte a ete verifie avec succes. Vous pouvez maintenant vous connecter.";
                    $success = true;
                }
            }
        } else {
            $message = "Token de verification invalide.";
        }
...
<head>
    ...
    <title>Verification du compte - Campus Event</title>
    ...
    <div class="verification-container">
        <div class="verification-card">
            ...
            <div class="card-body-custom">
                ...
                <h2 class="status-message">
                    <?php echo $success ? 'Verification Reussie !' : 'Erreur de Verification'; ?>
                </h2>
        ...
        <div class="text-center mt-4">
            <small style="color: rgba(255, 255, 255, 0.8);">
                © 2025 Campus Event. Tous droits reserves.
            </small>
        </div>
    </div>
